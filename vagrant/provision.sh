...
# install fpm
# sudo /opt/chef/embedded/bin/gem install fpm --no-ri --no-rdoc &&
# sudo ln -s /opt/chef/embedded/bin/fpm /usr/bin/fpm
# sudo ln -s /opt/chef/embedded/bin/fpm /usr/local/bin/fpm
...
# sudo pkm make -c logstash,elasticsearch,kibana3,nginx,rabbitmq,riemann,nodejs,openjdk,virtualenv,curl,make,ruby,workflow_gems -v &&
# sudo pkm pack -c cloudify-components
# sudo mv /cloudify/* ~/cloudify-packager/

# create aliases
# echo 'alias pip=/usr/local/bin/pip' >> /root/.bashrc
# echo 'alias python=/usr/local/bin/python2.7' >> /root/.bashrc
# echo 'alias virtualenv=/usr/local/bin/virtualenv-2.7' >> ~/.bashrc
# echo '. ~/packman/bin/activate' >> ~/.bashrc
# . /root/.bashrc
# # /usr/local/bin/virtualenv-2.7 packman &&
# # ~/packman/bin/pip install https://github.com/cloudify-cosmo/packman/archive/develop.tar.gz
